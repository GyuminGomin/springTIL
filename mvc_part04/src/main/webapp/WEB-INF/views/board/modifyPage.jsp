...
<script>
	// drag & drop 시 브라우저가 파일을 해석 하려는 기본이벤트를 제거
	...
	$(".fileDrop").on("drop", function(e) {
		...
		$.ajax({
			...
			contentType : false,
			processData : false,
			dataType : "json", // javascript객체로 바로 전달 받을 수 있음
			success : function(data) {
				// upload된 파일 이름을 list(배열) 로 전달
				// console.log(data);
	...
	$("#saveBtn").click(function() {
		...
		if (arr.length > 0) { // arr 배열에 삭제목록이 들어가 있으면
