package com.gyumin.di.controller.second;

import javax.annotation.PostConstruct;
import javax.annotation.Resource;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Qualifier;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.GetMapping;

import com.gyumin.di.dao.TestDAO;
import com.gyumin.di.service.TestService;

import lombok.extern.slf4j.Slf4j;

@Slf4j
@Controller
public class SecondController {
	
	@Resource
	private String path;

	@Autowired(required = false)
	TestService ts;
	
	@Autowired(required = false)
	@Qualifier(value = "td")
	TestDAO td;
	
	public SecondController() {
		log.info("SecondController 생성");
		log.info("second path : {}", path);
		log.info("second ts : {}", ts);
		log.info("second dao : {}", td);
	}
	
	/**
	 * bean 객체가 생성이 되고 나서 container에 의해 의존성 주입이 완료되고 객체가 사용될 준비가 완료되면
	 * bean으로 등록 되기 전에 최초에 한 번 호출 되는 method
	 */
	@PostConstruct
	public void init() {
		log.warn("PostConstruct init() 호출");
		log.info("PostConstruct path : {}", path);
		log.info("PostConstruct ts : {}", ts);
		log.info("PostConstruct dao : {}", td);
	}
	
	@GetMapping("main")
	public void doMain() {
		log.info("second controller do main 호출");
	}
}
