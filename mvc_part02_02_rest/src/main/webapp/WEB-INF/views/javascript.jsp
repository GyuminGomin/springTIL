...
<body>
	...
	<script>
		/*
			AJAX란 비동기 자바스크립트와 XML(Asynchronous JavaScript And Xml) 의 약어로
			비동기 형식으로 브라우저의 요청 변경없이 javascript와 서버 간의 통신을 하기 위한 XMLHttpRequest 객체를 사용하는 것을 말한다.
		*/
		...
		document.getElementById("continue_btn").onclick = function() {
			// continue_btn이 click 되었을 때 실행될 callback 함수
			// alert("click");
			httpRequest = new XMLHttpRequest();
			if (!httpRequest) {
				alert("AJAX 통신 불가");
				return false; // (js는 그냥 return하면 안됨)
			}
			...
			httpRequest.open('GET', 'sample2?name=김규민&age=26', false);
			// 등록된 정보로 server에 요청
			httpRequest.send();
